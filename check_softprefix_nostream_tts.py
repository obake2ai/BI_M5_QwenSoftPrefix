...
class StackFlowClient:
    ...
    def sys_reset(self) -> None:
        ...
        try:
            ...
            # 返ってくることもあるが、サーバが落ちてEOFになることもあるので握りつぶす
    ...
    def reset_and_wait_ready(self, max_wait_sec: float = 30.0) -> None:
        """
        sys.reset → 再接続 → pingが通るまで待つ
        sys.reset には「reset over」完了があるため、起動直後の race を避ける目的 :contentReference[oaicite:3]{index=3}
        """
    ...
    def audio_setup(self, playdevice: int = 1, play_volume: float = 0.7) -> str:
        """
        docs: playdevice=1 がオンボードスピーカ例 :contentReference[oaicite:4]{index=4}
        """
    ...
    def audio_setup_best_effort(self, play_volume: float = 0.7) -> Optional[str]:
        """
        -9(unit call false) はユニット呼び出し失敗 :contentReference[oaicite:5]{index=5}
        reset直後や音声デバイス準備中の可能性があるので数回リトライする
        """
        for attempt in range(1, 6):
            for dev in (1, 0):  # docでは1がspeaker :contentReference[oaicite:6]{index=6}
                try:
    ...
    def llm_setup_with_retry(self, model: str, system_prompt: str, max_token_len: int) -> str:
        """
        以前の 'task full' や setup固着を踏まえ、必要なら reset して再試行する
        """
    ...
    def tts_inference(self, tts_work_id: str, text: str, timeout_sec: float = 8.0) -> None:
        ...
        self.send_json(req)
        # sys.play.0_1 はACKが返らない/遅いことがあるので短めに待って抜ける
        try:
    ...
    def exit_work(self, work_id: str) -> None:
        # 一部環境で audio.exit が無いことがあるため、ここでは呼ぶだけで待たない
...
def main() -> None:
    ...
    # ★今回のポイント：TTSを使うときだけ audio/tts を触る
    ap.add_argument("--tts", action="store_true", help="デバイスTTSを有効化（この時だけ audio.setup も行う）")
    ap.add_argument("--tts-required", action="store_true", help="TTS初期化失敗時に終了する（デフォルトは生成だけ継続）")
    ap.add_argument("--tts-timeout", type=float, default=8.0)

    ap.add_argument("--audio-volume", type=float, default=0.7)

    # resetはデフォルトON（task full対策）。不要なら --no-reset
    ...
    # LLM setup 後に lsmode を取り直す（llm_setup_with_retry 内部で reset されうるため）
    lsmode = cli.sys_lsmode()
    ...
    if args.tts:
        # audio.setup（best effort + retry）
        audio_work_id = cli.audio_setup_best_effort(play_volume=args.audio_volume)
        if audio_work_id:
            print("[AUDIO SETUP OK] work_id:", audio_work_id)
        else:
            print("[WARN] audio.setup failed repeatedly. TTS may not play.")

        # 日本語TTS候補（lsmodeのmodeと、別名も一応トライ）
        candidates = [
            "melotts-ja-jp",
            "model-melotts-ja-jp",
        ]
        print("[TTS] candidates=", candidates, "response_format=", tts_resp)
        ...
        if tts_work_id is None:
            print("[WARN] Japanese TTS setup failed. Continue WITHOUT TTS.")
            # ここで英語フォールバックは“勝手に”しない（必要ならオプション追加できます）
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            # audioは work_id が audio.100x の時だけ exit するのが安全
